...
int main(int argc, char *argv[]) {
	...
	if (!inventory_create_connector(inv, conn_name, conn_arg, &conn)) {
		for (int i = 0; i < 1000 * 1000; i++) {
			...
			ConnectorInstance cloned = connectorinstance_clone(&conn);

			connectorinstance_drop(cloned);

			MemoryView phys_view = connectorinstance_phys_view(&conn);
			...
			memoryview_drop(phys_view);
		}

		connectorinstance_drop(conn);
		printf("conn dropped!\n");
	}
