...
pub fn find_lowstub(stub: &[u8]) -> Result<StartBlock> {
    stub.chunks_exact(architecture::x64::page_size_level(1).as_usize())
        .skip(1)
...
pub fn find(mem: &[u8]) -> Result<StartBlock> {
    mem.chunks_exact(architecture::x64::page_size_level(1).as_usize())
        ...
        .and_then(|i| {
            Ok(StartBlock {
                arch: Architecture::X64,
                va: Address::from(0),
                dtb: Address::from((i as u64) * architecture::x64::page_size_level(1).as_u64()),
            })
        })
}
