...
fn instantiate_connector(args: Option<&ConnectorArgs>) -> Result<impl PhysicalMemory + Clone> {
    ...
    let result = Inventory::scan().instantiate_connector("qemu", None, args)?;
...
fn win32_read_group(c: &mut Criterion) {
    ...
    phys::seq_read(c, "win32", &|| instantiate_connector(None));
    phys::chunk_read(c, "win32", &|| instantiate_connector(None));
