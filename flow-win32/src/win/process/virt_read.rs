...
impl<T: VirtualRead> ProcessRead for Process<T> {
    fn virt_read_addr(&mut self, addr: Address) -> Result<Address> {
        ...
        match proc_arch.instruction_set {
            InstructionSet::X64 => Ok(mem.virt_read_addr64(win.start_block.arch, dtb, addr)?),
            InstructionSet::X86 => Ok(mem.virt_read_addr32(win.start_block.arch, dtb, addr)?),
            _ => Err(Error::new("invalid process architecture")),
        }
    }
    ...
    fn virt_read_vec_addr(&mut self, addr: Address, count: usize) -> Result<Vec<Address>> {
        ...
        match proc_arch.instruction_set {
            InstructionSet::X64 => {
                Ok(mem.virt_read_vec_addr64(win.start_block.arch, dtb, addr, count)?)
            }
            InstructionSet::X86 => {
                Ok(mem.virt_read_vec_addr32(win.start_block.arch, dtb, addr, count)?)
            }
            _ => Err(Error::new("invalid process architecture")),
        }
    }
