...
use crate::arch::{self, GetArchitecture, Architecture};
...
// VirtualReadHelper + GetArchitecture will enable all read helpers
...
impl<'a, T: VirtualRead> VirtualReader<'a, T> {
    pub fn with(reader: &'a mut T, arch: Architecture, dtb: Address) -> Self {
        Self {
            reader,
            arch,
            dtb
...
impl<'a, T: VirtualRead> GetArchitecture for VirtualReader<'a, T> {
...
impl<'a, T: VirtualRead> VirtualReadHelper for VirtualReader<'a, T> {
    fn virt_read(&mut self, addr: Address, len: Length) -> Result<Vec<u8>> {
        self.reader.virt_read(self.arch, self.dtb, addr, len)
    }
}

pub trait VirtualReadHelperFuncs {
    unsafe fn virt_read_raw<U>(
        &mut self,
        addr: Address,
    ) -> Result<U>;
...
impl<T: VirtualReadHelper + GetArchitecture> VirtualReadHelperFuncs for T {
    unsafe fn virt_read_raw<U>(
        &mut self,
        addr: Address,
    ) -> Result<U> {
    ...
    fn virt_read_addr(
        ...
        let r = self.virt_read(addr, arch.instruction_set.len_addr())?;
    ...
    fn virt_read_vec_addr(
        ...
        let arch = self.architecture()?;
        let r = self.virt_read(addr, arch.instruction_set.len_addr() * count)?;
    ...
    fn virt_read_addr32(
        ...
        let arch = self.architecture()?;
        let r = self.virt_read(addr, arch.instruction_set.len_u32())?;
        Ok(Address::from(arch_read_type!(
            arch.instruction_set.byte_order(),
            read_u32,
            &r
    ...
    fn virt_read_vec_addr32(
        ...
        let arch = self.architecture()?;
        let r = self.virt_read(addr, arch.instruction_set.len_u32() * count)?;
        Ok(arch_read_vec_type!(
            arch.instruction_set.byte_order(),
    ...
    fn virt_read_addr64(
        ...
        let arch = self.architecture()?;
        let r = self.virt_read(addr, arch.instruction_set.len_u64())?;
        Ok(Address::from(arch_read_type!(
            arch.instruction_set.byte_order(),
            read_u64,
            &r
    ...
    fn virt_read_vec_addr64(
        ...
        let arch = self.architecture()?;
        let r = self.virt_read(addr, arch.instruction_set.len_u64() * count)?;
        Ok(arch_read_vec_type!(
            arch.instruction_set.byte_order(),
    ...
    fn virt_read_u16(&mut self, addr: Address) -> Result<u16> {
        let arch = self.architecture()?;
        let r = self.virt_read(addr, arch.instruction_set.len_u16())?;
        Ok(arch_read_type!(
            arch.instruction_set.byte_order(),
            read_u16,
            &r
    ...
    fn virt_read_u8(&mut self, addr: Address) -> Result<u8> {
        let arch = self.architecture()?;
        let r = self.virt_read(addr, arch.instruction_set.len_u8())?;
        Ok(r[0])
    }

    fn virt_read_i64(&mut self, addr: Address) -> Result<i64> {
        let arch = self.architecture()?;
        let r = self.virt_read(addr, arch.instruction_set.len_i64())?;
        Ok(arch_read_type!(
            arch.instruction_set.byte_order(),
            read_i64,
            &r
    ...
    fn virt_read_i32(&mut self, addr: Address) -> Result<i32> {
        let arch = self.architecture()?;
        let r = self.virt_read(addr, arch.instruction_set.len_i32())?;
        Ok(arch_read_type!(
            arch.instruction_set.byte_order(),
            read_i32,
            &r
    ...
    fn virt_read_i16(&mut self, addr: Address) -> Result<i16> {
        let arch = self.architecture()?;
        let r = self.virt_read(addr, arch.instruction_set.len_i16())?;
        Ok(arch_read_type!(
            arch.instruction_set.byte_order(),
            read_i16,
            &r
    ...
    fn virt_read_i8(&mut self, addr: Address) -> Result<i8> {
        let arch = self.architecture()?;
        let r = self.virt_read(addr, arch.instruction_set.len_i8())?;
        Ok(r[0] as i8)
    }

    fn virt_read_f32(&mut self, addr: Address) -> Result<f32> {
        let arch = self.architecture()?;
        let r = self.virt_read(addr, arch.instruction_set.len_f32())?;
        Ok(arch_read_type!(
            arch.instruction_set.byte_order(),
            read_f32,
            &r
    ...
    fn virt_read_vec_f32(
        ...
        let arch = self.architecture()?;
        let r = self.virt_read(addr, arch.instruction_set.len_f32() * count)?;
        Ok(arch_read_vec_type!(
            arch.instruction_set.byte_order(),
