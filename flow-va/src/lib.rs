...
// bit mask macros
macro_rules! make_bit_mask {
    ($a:expr, $b:expr) => ((0xffffffffffffffff >> (63 - $b)) & !(((1 as u64) << $a) - 1));
}

macro_rules! get_bit {
    ($a:expr, $b:expr) => (($a & ((1 as u64) << $b)) != 0);
}
...
macro_rules! pml4_index_bits {
    ($a:expr) => (($a & make_bit_mask!(39, 47)) >> 36);
}

macro_rules! pdpte_index_bits {
    ($a:expr) => (($a & make_bit_mask!(30, 38)) >> 27);
}

macro_rules! pd_index_bits {
    ($a:expr) => (($a & make_bit_mask!(21, 29)) >> 18);
}

macro_rules! pt_index_bits {
    ($a:expr) => (($a & make_bit_mask!(12, 20)) >> 9);
}
