...
pub fn find_lowstub(stub: &[u8]) -> Result<StartBlock> {
    stub.chunks_exact(x64::ARCH.page_size().try_into().unwrap())
        .skip(1)
        .filter(|c| {
...
pub fn find(mem: &[u8]) -> Result<StartBlock> {
    mem.chunks_exact(x64::ARCH.page_size().try_into().unwrap())
        .enumerate()
        .filter_map(|(i, c)| find_pt((i * x64::ARCH.page_size().try_into().unwrap()).into(), c))
        .map(|addr| StartBlock {
            arch: x64::ARCH.ident(),
            kernel_hint: 0.into(),
            dtb: addr,
